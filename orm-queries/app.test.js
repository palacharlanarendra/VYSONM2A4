...
describe("API Integration Tests v2", () => {
  ...
  it("should fail redirect with wrong password", async () => {
    ...
    expect(redirectResponse.body).toHaveProperty("error", "Password required or invalid");
  });
  ...
  it("Short code not existed", async () => {
    ...
    expect(redirectResponse.body).toHaveProperty("error", "Short code not found");
  });
  ...
  it("Short code is required", async () => {
    ...
    expect(redirectResponse.body).toHaveProperty("error", "Short code is required!");
  });

  it("should return 404 for unknown endpoint", async () => {
    const response = await request(app).get("/v2/test404");
  ...
  it("should update expiry date successfully", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.body).toHaveProperty("message", "Expiry date updated successfully");
...
describe("API Integration Tests v1 (Backward Compatibility)", () => {
  ...
  it("should redirect shortened URL without API key", async () => {
    ...
    expect(redirectResponse.statusCode).toBe(200);
    expect(redirectResponse.body).toHaveProperty("url", "https://example.com/v1");
  });
